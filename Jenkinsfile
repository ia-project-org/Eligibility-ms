             ...
             steps {
                 script {
                     ...
                     withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GITHUB_PASSWORD', usernameVariable: 'GITHUB_USERNAME')]) {
                         ...
                         sh """
                             git status
                             sed -i 's|image: minaouimh/ai:.*|image: minaouimh/ai:${imageTag}|g' deployment/eligibility-ms/eligibility-server-deployment.yaml
                             git config --global user.email "jenkins@example.com"
                             git config --global user.name "Jenkins"
                             git add deployment/eligibility-ms/eligibility-server-deployment.yaml
